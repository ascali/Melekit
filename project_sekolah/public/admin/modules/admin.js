...
$(document).ready(function(){
	 ...
	 table = $('#admin').DataTable( {
		...
		"order": [],
			"ajax": {
						"url": "http://localhost/project_sekolah/administrator/admin_data",
				        "type": "POST"
		        	},
		    "aoColumns": [
		    				{
                            ...
                            },
                            {
                              ...
                              var photo_user = row.foto;
                              var img_photo_user = photo_user ? '<img src="http://localhost/project_sekolah/public/admin/img/admin/'+photo_user+'" class="img-responsive" style="width:50%;">' : '';
...
function edit(id)
{
    ...
    $.ajax({
        url : "http://localhost/project_sekolah/administrator/admin_edit_data/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(dataRow)
        {
            ...
            $('.modal-title').text('Form Edit Admin'); // Set title to Bootstrap modal title
            ...
            if(dataRow.data.foto)
            {
                ...
                $('#foto div').html('<img src="'+base_url+'public/admin/img/admin/'+dataRow.data.foto+'" class="img-responsive" width="30%">'); // show photo
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "http://localhost/project_sekolah/administrator/admin_add";
    } else {
        url = "http://localhost/project_sekolah/administrator/admin_update";
    }
...
function del(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "http://localhost/project_sekolah/administrator/admin_delete/"+ id,
            type: "POST",
            dataType: "JSON",
            success: function(dataRow)
            {
