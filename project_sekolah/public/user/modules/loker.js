...
$(document).ready(function(){
    ...
    $.getJSON("http://localhost/project_sekolah/index/loker_data", function(json) {
        var content='';
        ...
        for (var a=0; a<json.data.length; a++) {
            ...
            content +=  '<article class="news-item page-row has-divider clearfix row">'+
                        ...
                        '<div class="details col-md-10 col-sm-9 col-xs-8">'+
                            ...
                            '<p>'+subisi+'</p>'+
                            '<a class="btn btn-theme read-more" href="#" onclick="detail_loker('+id+',true);">Read more<i class="fa fa-chevron-right"></i></a>'+
                        '</div>'+
                    '</article>';

            $('.isi_loker').html(content);
    ...
    $.getJSON("http://localhost/project_sekolah/index/loker_data", function(data) {
        var items = [];
...
function detail_loker(id)
{
    ...
    $.ajax({
        url : "http://localhost/project_sekolah/index/detail_loker_data/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(dataRow)
        {
            //success: function(response) {
                var page = "http://localhost/project_sekolah/index/detail_loker/" + id;
                // id;
